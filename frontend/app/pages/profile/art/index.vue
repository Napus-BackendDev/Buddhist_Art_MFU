<template>
    <div class="flex flex-col px-10 w-full">
        <header class="justify-between items-center space-y-5 mb-8">
            <div class="flex flex-col items-center gap-2 my-8">
                <h1 class="text-5xl font-extrabold text-red-600 text-center">ผลงานศิลปะของฉัน</h1>
            ...
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-gray-800">
                    ผลงานทั้งหมด
                    <span class="text-red-600">{{ artworksByUser?.length || 0 }}</span>
                    ชิ้น
                </h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 iteams-stretch gap-8 w-full">
            <NuxtLink v-for="art in filteredArtworks" :key="art._id">
                <ArtworkCard :art="art" :button-sell="false" :editor="true" @delete="handleArtworkDelete" />
            </NuxtLink>
        </div>
...
const { artworksByUser, fetchArtworksByUser } = useFetchArtworksByUser(user.value?.username.th || '');
...
const categoryOptions = computed(() => {
    const types = [...new Set((artworksByUser.value || []).map(a => a.type))].filter(Boolean)
