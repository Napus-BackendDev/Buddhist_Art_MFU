...
import { ShoppingCart, Settings, Trash } from 'lucide-vue-next';
...
<template>
    <div class="relative rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-lg transition-shadow duration-300">
        ...
        <RouterLink v-if="props.art?._id" :to="{ name: 'artworkDetail', params: { id: art._id } }">
            <div class="relative">
                ...
                <div v-if="props.showEditButton" class="absolute top-2 right-2 flex gap-2">
                    <button @click.prevent.stop="handleEdit"
                        class="bg-blue-500 backdrop-blur-sm text-white p-2.5 rounded-xl shadow-lg transition-all hover:scale-110">
                        <Settings :size="20" />
                    </button>
                    <button @click.prevent.stop="handleDelete"
            ...
            <div class="p-5">
                ...
                <div class="text-sm text-gray-600 grid grid-cols gap-2 pt-2">
                ...
                <!-- <div class="text-sm text-gray-600 grid grid-cols-2 gap-2 pt-2">
                    ...
                    <div><span class="font-medium">เทคนิค:</span> {{ props.art?.technique?.th || props.art?.technique ||'-' }}</div>
                    <div><span class="font-medium">การนำเสนอ:</span> {{ props.art?.presentation?.th|| props.art?.presentation || '-' }}</div>
                </div> -->
                <button v-if="showAddToCart" @click.prevent.stop="handleAddToCart"
                    class="mt-5 w-full px-4 py-3 rounded-lg flex items-center justify-center gap-2 transition-all"
                    ...
                    <span v-else-if="props.isInCart" class="flex items-center gap-1">✓ อยู่ในตะกร้าแล้ว
                        (คลิกเพื่อลบ)</span>
